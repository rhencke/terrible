...
page_title: "terrible_cisco_dnac_network_device_config_files_count_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_dnac_network_device_config_files_count_info (Resource)
...
- `fileType` (String) FileType query parameter. Type of device configuration file. Available values 'RUNNINGCONFIG', 'STARTUPCONFIG', 'VLAN'.
- `headers` (String) Additional headers.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `networkDeviceId` (String) NetworkDeviceId query parameter. Unique identifier (UUID) of the network devices. The number of networkDeviceId(s) must not exceed 5.
