...
page_title: "terrible_cisco_intersight_intersight_smtp_policy Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_intersight_intersight_smtp_policy (Resource)
...
- `name` (String) The name assigned to the SMTP Policy. The name must be between 1 and 62 alphanumeric characters, allowing special characters :-_.
...
- `description` (String) The user-defined description for the SMTP Policy. Description can contain letters(a-z, A-Z), numbers(0-9), hyphen(-), period(.), colon(:), or an underscore(_).
- `enabled` (Boolean) If enabled, controls the state of the SMTP client service on the managed device.
...
- `smtp_port` (Number) Port number used by the SMTP server for outgoing SMTP communication. Valid range is 1-65535.
- `smtp_recipients` (String) List of email addresses that will receive notifications for faults.
- `smtp_server` (String) IP address or hostname of the SMTP server. The SMTP server is used by the managed device to send email notifications. Required when `enabled` is True.
