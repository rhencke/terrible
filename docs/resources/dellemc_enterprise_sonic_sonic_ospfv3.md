...
page_title: "terrible_dellemc_enterprise_sonic_sonic_ospfv3 Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_dellemc_enterprise_sonic_sonic_ospfv3 (Resource)
...
- `config` (String) Specifies the OSPFv3 related configuration.
...
- `state` (String) Specifies the operation to be performed on the OSPFv3 process configured on the device. In case of merged, the input configuration will be merged with the existing OSPFv3 configuration on the device. In case of deleted, the specified existing OSPFv3 configuration will be removed from the device. In case of overridden, all the existing OSPFv3 configuration will be deleted and the specified input configuration will be installed. In case of replaced, the existing OSPFv3 configuration on the device will be replaced by the configuration in the playbook for each VRF group configured by the playbook.
