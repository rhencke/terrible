...
page_title: "terrible_infoblox_nios_modules_nios_cname_record Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_infoblox_nios_modules_nios_cname_record (Resource)
...
- `canonical` (String) Configures the canonical name for this CNAME record.
- `host_id` (String) ID of the `terrible_host` to run this task against
- `name` (String) Specifies the fully qualified hostname to add or remove from the system. User can also update the name as it is possible to pass a dict containing *new_name*, *old_name*. See examples
...
- `ttl` (Number) Configures the TTL to be associated with this CNAME record.
- `view` (String) Sets the DNS view to associate this CNAME record with.  The DNS view must already be configured on the system.
