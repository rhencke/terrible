...
page_title: "terrible_azure_azcollection_azure_rm_mysqlserver Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_azure_azcollection_azure_rm_mysqlserver (Resource)
...
- `restarted` (Boolean) Set to `true` with *state=present* to restart a running mysql server.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `sku` (String) The SKU (pricing tier) of the server.
- `state` (String) Assert the state of the MySQL Server. Use `present` to create or update a server and `absent` to delete it.
- `storage_profile` (String) Storage Profile properties of a server.
