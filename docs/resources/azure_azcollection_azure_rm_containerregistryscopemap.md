...
page_title: "terrible_azure_azcollection_azure_rm_containerregistryscopemap Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_azure_azcollection_azure_rm_containerregistryscopemap (Resource)
...
- `name` (String) The name of the container registry scope map.
...
- `actions` (String) The list of scoped permissions for registry artifacts. Sample as `repositories/repository-name/content/read` or `repositories/repository-name/metadata/write`.
...
- `state` (String) Assert the state of the container registry scope map. Use `present` to create or update a server and `absent` to delete it.
...
- `scope_map` (String) A list of dictionaries containing facts for scope map.
