...
page_title: "terrible_ovirt_ovirt_ovirt_host_storage_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_ovirt_ovirt_ovirt_host_storage_info (Resource)
...
- `host` (String) Host to get device list from.
...
- `fcp` (String) Dictionary with values for fibre channel storage type:
- `follow` (String) List of linked entities, which should be fetched along with the main entity. This parameter replaces usage of `fetch_nested` and `nested_attributes`. All follow parameters can be found at following url: https://ovirt.github.io/ovirt-engine-api-model/master/#types/host_storage/links_summary
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `iscsi` (String) Dictionary with values for iSCSI storage type:
...
- `ovirt_host_storages` (String) List of dictionaries describing the HostStorage. HostStorage attributes are mapped to dictionary keys, all HostStorage attributes can be found at following url: http://ovirt.github.io/ovirt-engine-api-model/master/#types/host_storage.
