...
page_title: "terrible_purestorage_flashblade_purefb_network Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_purestorage_flashblade_purefb_network (Resource)
...
- `itype` (String) Type of interface.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `services` (String) Define which services are configured for the interfaces.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) Create, delete or modifies a network interface.
