...
page_title: "terrible_cisco_dnac_http_read_credential Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_dnac_http_read_credential (Resource)
...
- `comments` (String) Comments to identify the HTT`S` Read credential.
- `credentialType` (String) Credential type to identify the application that uses the HTT`S` Read credential.
- `delegate_to_id` (String) ID of another terrible_host to delegate execution to.
- `description` (String) Description for HTT`S` Read Credential.
...
- `password` (String) HTT`S` Read Password.
...
- `secure` (Boolean) Flag for HTTPS Read.
...
- `username` (String) HTT`S` Read Username.
