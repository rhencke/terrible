...
page_title: "terrible_azure_azcollection_azure_rm_devtestlabpolicy Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_azure_azcollection_azure_rm_devtestlabpolicy (Resource)
...
- `policy_set_name` (String) The name of the policy set.
...
- `fact_name` (String) The fact name of the policy (e.g. `lab_vm_count`, `lab_vm_size`), MaxVmsAllowedPerLab, etc.
...
- `state` (String) Assert the state of the Policy. Use `present` to create or update an Policy and `absent` to delete it.
- `tags` (String) Run only tasks with these Ansible tags (list of strings).
- `threshold` (String) The threshold of the policy (it could be either a maximum value or a list of allowed values).
