...
page_title: "terrible_check_point_mgmt_cp_mgmt_time Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_check_point_mgmt_cp_mgmt_time (Resource)
...
- `end` (String) End time. Note, Each gateway may interpret this time differently according to its time zone.
- `end_never` (Boolean) End never.
...
- `hours_ranges` (String) Hours recurrence. Note, Each gateway may interpret this time differently according to its time zone.
...
- `recurrence` (String) Days recurrence.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `start` (String) Starting time. Note, Each gateway may interpret this time differently according to its time zone.
...
- `cp_mgmt_time` (String) The checkpoint object created or updated.
