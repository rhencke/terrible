...
page_title: "terrible_community_proxmox_proxmox_backup_schedule Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_proxmox_proxmox_backup_schedule (Resource)
...
- `state` (String) If `present`, ensure that VM is present in the backup job on defined `backup_id`. If `absent`, ensure that VM is not present in the backup job if `backup_id` is defined and otherwise, it should not exist in any of the backup jobs.

### Optional

- `async_seconds` (Number) Run the task asynchronously, timing out after this many seconds. 0 = synchronous (default).
- `backup_id` (String) The backup job ID.
...
- `vm_id` (String) The ID of the Proxmox VM. Mutually exclusive with `vm_name`.
- `vm_name` (String) The name of the Proxmox VM. Mutually exclusive with `vm_id`.
