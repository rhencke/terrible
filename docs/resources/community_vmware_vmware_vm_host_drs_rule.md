...
page_title: "terrible_community_vmware_vmware_vm_host_drs_rule Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_vmware_vmware_vm_host_drs_rule (Resource)
...
- `cluster_name` (String) Cluster to create VM-Host rule.
- `drs_rule_name` (String) Name of rule to create or remove.
...
- `host_group_name` (String) Name of Host group to use with rule. Effective only if `state=present`.
...
- `state` (String) If set to `present` and the rule does not exist then the rule will be created. If set to `absent` and the rule exists then the rule will be deleted.
...
- `vm_group_name` (String) Name of VM group to use with rule. Effective only if `state=present`.
