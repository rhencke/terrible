...
page_title: "terrible_netapp_ontap_na_ontap_ntp_key Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_netapp_ontap_na_ontap_ntp_key (Resource)
...
- `digest_type` (String) NTP symmetric authentication key type. Only SHA1 currently supported.
- `host_id` (String) ID of the `terrible_host` to run this task against
- `value` (String) NTP symmetric authentication key value. The value must be exactly 40 hexadecimal digits for SHA1 keys.
...
- `state` (String) Whether the specified NTP key should exist or not.
