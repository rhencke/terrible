...
page_title: "terrible_amazon_aws_autoscaling_instance_refresh_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_amazon_aws_autoscaling_instance_refresh_info (Resource)
...
- `ids` (String) One or more instance refresh IDs.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `max_records` (Number) The maximum number of items to return with this call. The default value is `50` and the maximum value is `100`.
...
- `instance_refreshes` (String) A list of instance refreshes.
