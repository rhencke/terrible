...
page_title: "terrible_openstack_cloud_volume_snapshot Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_openstack_cloud_volume_snapshot (Resource)
...
- `description` (String) String describing the snapshot
...
- `force` (Boolean) Allows or disallows snapshot of a volume to be created, when the volume is attached to an instance.
...
- `state` (String) Should the snapshot be `present` or `absent`.
...
- `volume` (String) Volume name or ID to create the snapshot from. Required when *state* is `present`.

### Read-Only

- `changed` (Boolean) Whether the task reported a change
- `id` (String) Unique task resource ID
- `snapshot` (String) Same as `volume_snapshot`, kept for backward compatibility.
- `volume_snapshot` (String) The snapshot instance
