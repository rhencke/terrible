...
page_title: "terrible_community_zabbix_zabbix_user_role Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_zabbix_zabbix_user_role (Resource)
...
- `name` (String) Name of the role to be processed
...
- `rules` (String) Rules set as defined in https://www.zabbix.com/documentation/current/en/manual/api/reference/role/object#role-rules
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) State of the user_role. On `present`, it will create if user_role does not exist or update the user_role if the associated data is different. On `absent` will remove a user_role if it exists.
...
- `msg` (String) The result of the action
