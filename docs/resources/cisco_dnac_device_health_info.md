...
page_title: "terrible_cisco_dnac_device_health_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_dnac_device_health_info (Resource)
...
- `deviceRole` (String) DeviceRole query parameter. CORE, ACCESS, DISTRIBUTION, ROUTER, WLC, or AP (case insensitive).
- `endTime` (Number) EndTime query parameter. UTC epoch time in milliseconds.
...
- `siteId` (String) SiteId query parameter. DNAC site UUID.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `startTime` (Number) StartTime query parameter. UTC epoch time in milliseconds.
