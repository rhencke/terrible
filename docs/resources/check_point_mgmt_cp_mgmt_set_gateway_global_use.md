...
page_title: "terrible_check_point_mgmt_cp_mgmt_set_gateway_global_use Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_check_point_mgmt_cp_mgmt_set_gateway_global_use (Resource)
...
- `enabled` (Boolean) Indicates whether global use is enabled on the target.
- `host_id` (String) ID of the `terrible_host` to run this task against
- `target` (String) On what target to execute this command. Target may be identified by its object name, or object unique identifier.
...
- `cp_mgmt_set_gateway_global_use` (String) The checkpoint set-gateway-global-use output.
