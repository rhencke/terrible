...
page_title: "terrible_cisco_meraki_organizations_camera_boundaries_lines_by_device_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_meraki_organizations_camera_boundaries_lines_by_device_info (Resource)
...
- `organizationId` (String) Information module for Organizations Camera Boundaries Lines Bydevice Info. OrganizationId path parameter. Organization ID.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `serials` (String) Information module for Organizations Camera Boundaries Lines Bydevice Info. Serials query parameter. A list of serial numbers. The returned cameras will be filtered to only include these serials.
