...
page_title: "terrible_community_zabbix_zabbix_item Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_zabbix_zabbix_item (Resource)
...
- `name` (String) Name of item to create or delete.
...
- `host_name` (String) Name of host to add item to. Required when *template_name* is not used. Mutually exclusive with *template_name*.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `params` (String) Parameters to create/update item with. Required if state is "present". Parameters as defined at https://www.zabbix.com/documentation/current/en/manual/api/reference/item/object Additionally supported parameters are below
...
- `state` (String) Create or delete item.
- `tags` (String) Run only tasks with these Ansible tags (list of strings).
- `template_name` (String) Name of template to add item to. Required when *host_name* is not used. Mutually exclusive with *host_name*.
