...
page_title: "terrible_google_cloud_gcp_serviceusage_service Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_google_cloud_gcp_serviceusage_service (Resource)
...
- `disable_dependent_services` (Boolean) Indicates if dependent services should also be disabled. Can only be turned on if service is disabled.
...
- `config` (String) The service configuration of the available service.
- `disableDependentServices` (Boolean) Indicates if dependent services should also be disabled. Can only be turned on if service is disabled.
- `id` (String) Unique task resource ID
- `parent` (String) The name of the parent of this service. For example 'projects/123' .
