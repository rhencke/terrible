...
page_title: "terrible_cisco_dnac_pnp_virtual_account_add Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_dnac_pnp_virtual_account_add (Resource)
...
- `autoSyncPeriod` (Number) Pnp Virtual Account Add's autoSyncPeriod.
...
- `expiry` (Number) Pnp Virtual Account Add's expiry.
...
- `lastSync` (Number) Pnp Virtual Account Add's lastSync.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `profile` (String) Pnp Virtual Account Add's profile.
...
- `syncResult` (String) Pnp Virtual Account Add's syncResult.
- `syncResultStr` (String) Represent internal state and SHOULD not be used or relied upon. (Deprecated).
- `syncStartTime` (Number) Pnp Virtual Account Add's syncStartTime.
- `syncStatus` (String) Represent internal state and SHOULD not be used or relied upon. (Deprecated).
- `tags` (String) Run only tasks with these Ansible tags (list of strings).
- `tenantId` (String) Represent internal state and SHOULD not be used or relied upon. (Deprecated).
