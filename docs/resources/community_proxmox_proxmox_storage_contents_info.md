...
page_title: "terrible_community_proxmox_proxmox_storage_contents_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_proxmox_proxmox_storage_contents_info (Resource)
...
- `node` (String) Proxmox node to which the storage is attached.
- `storage` (String) Only return content stored on that specific storage.
...
- `content` (String) Filter on a specific content type.
...
- `vmid` (Number) Filter on a specific VMID.

### Read-Only

- `changed` (Boolean) Whether the task reported a change
- `id` (String) Unique task resource ID
- `proxmox_storage_content` (String) Content of of storage attached to a node.
