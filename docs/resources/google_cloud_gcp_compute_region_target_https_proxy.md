...
page_title: "terrible_google_cloud_gcp_compute_region_target_https_proxy Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_google_cloud_gcp_compute_region_target_https_proxy (Resource)
...
- `sslCertificates` (String) A list of RegionSslCertificate resources that are used to authenticate connections between users and the load balancer. Currently, exactly one SSL certificate must be specified.
- `urlMap` (String) A reference to the RegionUrlMap resource that defines the mapping from URL to the RegionBackendService.
