...
page_title: "terrible_cisco_mso_ndo_schema_template_deploy Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_mso_ndo_schema_template_deploy (Resource)
...
- `sites` (String) The name of the site(s).
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) Use `deploy` to deploy schema template. Use `redeploy` to redeploy schema template. Use `undeploy` to undeploy schema template from a site. Use `query` to get deployment status.
