...
page_title: "terrible_infoblox_nios_modules_nios_dtc_topology Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_infoblox_nios_modules_nios_dtc_topology (Resource)
...
- `name` (String) Specifies the DTC Topology display name.
...
- `rules` (String) Configures the topology rules
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) Configures the intended state of the instance of the object on the NIOS topology.  When this value is set to `present`, the object is configured on the device and when this value is set to `absent` the value is removed (if necessary) from the device.
