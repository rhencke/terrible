...
page_title: "terrible_cisco_dnac_configuration_template_import_template Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_dnac_configuration_template_import_template (Resource)
...
- `doVersion` (Boolean) DoVersion query parameter. If this flag is true then it creates a new version of the template with the imported contents in case if the templates already exists. " If this flag is false and if template already exists, then operation fails with 'Template already exists' error.
...
- `payload` (String) Configuration Template Import Template's payload.
