...
page_title: "terrible_community_general_jenkins_build_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_general_jenkins_build_info (Resource)
...
- `name` (String) Name of the Jenkins job to which the build belongs.

### Optional

- `async_seconds` (Number) Run the task asynchronously, timing out after this many seconds. 0 = synchronous (default).
- `build_number` (Number) An integer which specifies a build of a job. If not specified the last build information is returned.
...
- `state` (String) State of the jenkins job.
