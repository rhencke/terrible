...
page_title: "terrible_ieisystem_inmanage_update_fw Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_ieisystem_inmanage_update_fw (Resource)
...
- `dual_image` (String) Update dual image(default) or not. Only the M5 model supports this parameter.
...
- `has_me` (Number) Update me or not when update bios, only work in INTEL platform, 0-no, 1-yes. Only the M5 model supports this parameter.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `mode` (String) Active mode, Manual or Auto(default).
- `over_ride` (Number) Reserve Configurations, 0-reserve, 1-override.
