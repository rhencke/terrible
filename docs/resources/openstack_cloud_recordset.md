...
page_title: "terrible_openstack_cloud_recordset Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_openstack_cloud_recordset (Resource)
...
- `name` (String) Name of the recordset. It must be ended with name of dns zone.
- `zone` (String) Name or ID of the zone which manages the recordset
...
- `records` (String) List of recordset definitions. Required when *state=present*.
- `recordset_type` (String) Recordset type Required when *state=present*.
...
- `ttl` (Number) TTL (Time To Live) value in seconds

### Read-Only

- `changed` (Boolean) Whether the task reported a change
- `id` (String) Unique task resource ID
- `recordset` (String) Dictionary describing the recordset.
