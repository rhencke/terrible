...
page_title: "terrible_check_point_mgmt_cp_mgmt_threat_protection_override Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_check_point_mgmt_cp_mgmt_threat_protection_override (Resource)
...
- `follow_up` (Boolean) Tag the protection with pre-defined follow-up flag.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `name` (String) Object name.
- `overrides` (String) Overrides per profile for this protection<br> Note, Remove override for Core protections removes only the action's override. Remove override for Threat Cloud protections removes the action, track and packet captures.
...
- `cp_mgmt_threat_protection_override` (String) The checkpoint threat_protection_override output.
