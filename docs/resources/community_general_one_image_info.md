...
page_title: "terrible_community_general_one_image_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_general_one_image_info (Resource)
...
- `ids` (String) A list of images IDs whose facts you want to gather. Module can use integers too.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `name` (String) A `name` of the image whose facts is gathered. If the `name` begins with `~` the `name` is used as regex pattern, which restricts the list of images (whose facts is returned) whose names match specified regex. Also, if the `name` begins with `~*` case-insensitive matching is performed. See examples for more details.
...
- `images` (String) A list of images info.
