...
page_title: "terrible_cisco_iosxr_iosxr_system Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_iosxr_iosxr_system (Resource)
...
- `lookup_enabled` (Boolean) Provides administrative control for enabling or disabling DNS lookups.  When this argument is set to True, lookups are performed and when it is set to False, lookups are not performed.
...
- `name_servers` (String) The `name_serves` argument accepts a list of DNS name servers by way of either FQDN or IP address to use to perform name resolution lookups.  This argument accepts wither a list of DNS servers See examples.
...
- `vrf` (String) VRF name for domain services
