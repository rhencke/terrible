---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "terrible_hetzner_hcloud_floating_ip Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_hetzner_hcloud_floating_ip (Resource)





<!-- schema generated by tfplugindocs -->
## Schema

### Required

- `host_id` (String) ID of the `terrible_host` to run this task against

### Optional

- `async_seconds` (Number) Run the task asynchronously, timing out after this many seconds. 0 = synchronous (default).
- `changed_when` (String) Jinja2 expression that overrides when the task is considered changed (e.g. 'false').
- `delegate_to_id` (String) ID of another terrible_host to delegate execution to.
- `delete_protection` (Boolean) Protect the Floating IP for deletion.
- `description` (String) The Description of the Hetzner Cloud Floating IPs.
- `environment` (String) Environment variables set for the task (dict of name→value).
- `failed_when` (String) Jinja2 expression that overrides when the task is considered failed.
- `force` (Boolean) Force the assignment or deletion of the Floating IP.
- `home_location` (String) Home Location of the Hetzner Cloud Floating IP. Required if no *server* is given and Floating IP does not exist.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `labels` (String) User-defined labels (key-value pairs).
- `name` (String) The Name of the Hetzner Cloud Floating IPs to manage. Only required if no Floating IP *id* is given or a Floating IP does not exist.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `server` (String) Server Name the Floating IP should be assigned to. Required if no *home_location* is given and Floating IP does not exist.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) State of the Floating IP.
- `tags` (String) Run only tasks with these Ansible tags (list of strings).
- `timeout` (Number) Override the default execution timeout (seconds). Defaults to 300.
- `triggers` (String) Arbitrary map of values; any change triggers task re-execution
- `type` (String) Type of the Floating IP. Required if Floating IP does not exist

### Read-Only

- `changed` (Boolean) Whether the task reported a change
- `hcloud_floating_ip` (String) The Floating IP instance
- `id` (String) Unique task resource ID
