...
page_title: "terrible_openstack_cloud_keypair Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_openstack_cloud_keypair (Resource)
...
- `name` (String) Name that has to be given to the key pair
...
- `state` (String) Should the resource be present or absent. If state is replace and the key exists but has different content, delete it and recreate it with the new content.
...
- `keypair` (String) Dictionary describing the keypair.
