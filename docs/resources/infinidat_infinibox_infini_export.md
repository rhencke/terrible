...
page_title: "terrible_infinidat_infinibox_infini_export Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_infinidat_infinibox_infini_export (Resource)
...
- `filesystem` (String) Name of exported file system.
- `host_id` (String) ID of the `terrible_host` to run this task against
- `name` (String) Export name. Must start with a forward slash, e.g. name=/data.
...
- `client_list` (String) List of dictionaries with client entries. See examples. Check infini_export_client module to modify individual NFS client entries for export.
...
- `state` (String) Creates/Modifies export when present, removes when absent, or provides export details with stat.
