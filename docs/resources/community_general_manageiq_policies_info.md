...
page_title: "terrible_community_general_manageiq_policies_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_general_manageiq_policies_info (Resource)
...
- `resource_type` (String) The type of the resource to obtain the profile for.
...
- `resource_id` (Number) The ID of the resource to obtain the profile for. Must be specified if `resource_name` is not set. Both options are mutually exclusive.
...
- `profiles` (String) List current policy_profile and policies for a provider in ManageIQ.
