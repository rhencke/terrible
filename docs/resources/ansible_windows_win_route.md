...
page_title: "terrible_ansible_windows_win_route Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_ansible_windows_win_route (Resource)
...
- `destination` (String) Destination IP address in CIDR format (ip address/prefix length).
...
- `metric` (Number) Metric used by the static route.
...
- `state` (String) If `absent`, it removes a network static route. If `present`, it adds a network static route.
...
- `msg` (String) A message describing the task result.
