---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "terrible_azure_azcollection_azure_rm_iothub_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_azure_azcollection_azure_rm_iothub_info (Resource)





<!-- schema generated by tfplugindocs -->
## Schema

### Required

- `host_id` (String) ID of the `terrible_host` to run this task against

### Optional

- `async_seconds` (Number) Run the task asynchronously, timing out after this many seconds. 0 = synchronous (default).
- `changed_when` (String) Jinja2 expression that overrides when the task is considered changed (e.g. 'false').
- `delegate_to_id` (String) ID of another terrible_host to delegate execution to.
- `environment` (String) Environment variables set for the task (dict of name→value).
- `failed_when` (String) Jinja2 expression that overrides when the task is considered failed.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `list_consumer_groups` (Boolean) List the consumer group of the built-in event hub.
- `list_keys` (Boolean) List the keys of IoT Hub. Note this will have network overhead for each IoT Hub.
- `name` (String) Limit results to a specific resource group.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `resource_group` (String) The resource group to search for the desired IoT Hub.
- `show_endpoint_health` (Boolean) Get the health for routing endpoints. Note this will have network overhead for each IoT Hub.
- `show_quota_metrics` (Boolean) Get the quota metrics for an IoT hub. Note this will have network overhead for each IoT Hub.
- `show_stats` (Boolean) Show the statistics for IoT Hub. Note this will have network overhead for each IoT Hub.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `tags` (String) Run only tasks with these Ansible tags (list of strings).
- `test_route_message` (String) Test routes message. It will be used to test all routes.
- `timeout` (Number) Override the default execution timeout (seconds). Defaults to 300.
- `triggers` (String) Arbitrary map of values; any change triggers task re-execution

### Read-Only

- `azure_iothubs` (String) List of IoT Hub dicts.
- `changed` (Boolean) Whether the task reported a change
- `id` (String) Unique task resource ID
