...
page_title: "terrible_community_general_hponcfg Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_general_hponcfg (Resource)
...
- `path` (String) The XML file as accepted by `hponcfg`.
...
- `executable` (String) Path to the hponcfg executable (`hponcfg` which uses `PATH`).
...
- `verbose` (Boolean) Run `hponcfg` in verbose mode (-v).
