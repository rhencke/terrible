...
page_title: "terrible_ibm_storage_virtualize_ibm_svc_manage_cv Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_ibm_storage_virtualize_ibm_svc_manage_cv (Resource)
...
- `cvname` (String) Specifies the name to assign to the master or auxiliary change volume.
- `host_id` (String) ID of the `terrible_host` to run this task against
- `rname` (String) Specifies the name of the remote copy relationship.
- `state` (String) Creates or updates (`present`) or removes (`absent`), a change volume.
...
- `ismaster` (Boolean) Specifies whether the change volume is being (dis)associated with master cluster. Required when the change volume is being associated or disassociated from the master cluster.
