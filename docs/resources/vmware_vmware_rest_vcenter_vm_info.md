...
page_title: "terrible_vmware_vmware_rest_vcenter_vm_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_vmware_vmware_rest_vcenter_vm_info (Resource)
...
- `vm` (String) Virtual machine identifier. The parameter must be the id of a resource returned by `vmware.vmware_rest.vcenter_vm_info`. Required with *state=['get']*
- `vms` (String) Identifiers of virtual machines that can match the filter. If unset or empty, virtual machines with any identifier match the filter. When clients pass a value of this structure as a parameter, the field must contain the id of resources returned by `vmware.vmware_rest.vcenter_vm_info`.

### Read-Only

- `changed` (Boolean) Whether the task reported a change
- `id` (String) Unique task resource ID
- `value` (String) Wait until my VM is off
