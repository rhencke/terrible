...
page_title: "terrible_ieisystem_inmanage_ldap_group Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_ieisystem_inmanage_ldap_group (Resource)
...
- `kvm` (String) Kvm privilege.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `pri` (String) Group privilege.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) Whether the ldap group should exist or not, taking action if the state is different from what is stated.
