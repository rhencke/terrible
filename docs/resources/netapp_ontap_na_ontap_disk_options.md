...
page_title: "terrible_netapp_ontap_na_ontap_disk_options Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_netapp_ontap_na_ontap_disk_options (Resource)
...
- `node` (String) The node to modify a disk option for

### Optional

- `async_seconds` (Number) Run the task asynchronously, timing out after this many seconds. 0 = synchronous (default).
- `autoassign` (Boolean) Whether or not disks should be automatically assigned to a node
- `autoassign_policy` (String) the auto assign policy to use
- `autocopy` (Boolean) Whether or not disk auto copies should be enabled
- `bkg_firmware_update` (Boolean) Whether or not background disk firmware updates should be enabled
