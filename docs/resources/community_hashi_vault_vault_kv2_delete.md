...
page_title: "terrible_community_hashi_vault_vault_kv2_delete Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_hashi_vault_vault_kv2_delete (Resource)
...
- `path` (String) Vault KV path to be deleted. This is relative to the *engine_mount_point*, so the mount path should not be included. For kv2, do not include `/data/` or `/metadata/`.
...
- `versions` (String) One or more versions of the secret to delete. When omitted, the latest version of the secret is deleted.

### Read-Only

- `changed` (Boolean) Whether the task reported a change
- `data` (String) The raw result of the delete against the given path. This is usually empty, but may contain warnings or other information.
