...
page_title: "terrible_purestorage_flashblade_purefb_lag Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_purestorage_flashblade_purefb_lag (Resource)
...
- `ports` (String) Name of network ports assigned to the LAG Format should be CHx.ETHy, where CHx is the chassis number and ETHy is the ethernet port number. Matched port pairs from each Fabric Module in the Chassis will be used. To modify required ports for a LAG specify only the ports required by the LAG. Any ports currently used by the LAG not specified will be disconnected from the LAG.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) Define whether the LAG should be added or deleted
...
- `lag` (String) A dictionary describing the LAG.
