...
page_title: "terrible_cisco_dnac_provisioning_settings Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_dnac_provisioning_settings (Resource)
...
- `requireItsmApproval` (Boolean) If require ITSM approval is enabled, the planned configurations must be submitted for ITSM approval. Also if enabled, requirePreview will default to enabled.
- `requirePreview` (Boolean) If require preview is enabled, the device configurations must be reviewed before deploying them.
