...
page_title: "terrible_community_windows_win_rds_settings Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_windows_win_rds_settings (Resource)
...
- `enable_only_messaging_capable_clients` (Boolean) If enabled, only clients that support logon messages and administrator messages can connect.
...
- `max_connections` (Number) The maximum number of connections allowed. If set to `0`, no new connections are allowed. If set to `-1`, the number of connections is unlimited.
...
- `ssl_bridging` (String) Specifies whether to use SSL Bridging. `none`: no SSL bridging. `https_http`: HTTPS-HTTP bridging. `https_https`: HTTPS-HTTPS bridging.
