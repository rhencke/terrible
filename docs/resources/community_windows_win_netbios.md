...
page_title: "terrible_community_windows_win_netbios Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_windows_win_netbios (Resource)
...
- `state` (String) Whether NetBIOS should be enabled, disabled, or default (use setting from DHCP server or if static IP address is assigned enable NetBIOS).

### Optional

- `adapter_names` (String) List of adapter names for which to manage NetBIOS settings. If this option is omitted then configuration is applied to all adapters on the system. The adapter name used is the connection caption in the Network Control Panel or via `Get-NetAdapter`, eg `Ethernet 2`.
...
- `reboot_required` (Boolean) Boolean value stating whether a system reboot is required.
