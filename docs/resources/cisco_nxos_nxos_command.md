...
page_title: "terrible_cisco_nxos_nxos_command Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_nxos_nxos_command (Resource)
...
- `retries` (Number) Specifies the number of retries a command should by tried before it is considered failed. The command is run on the target device every retry and evaluated against the *wait_for* conditionals. The commands are run once when *retries* is set to `0`.
