...
page_title: "terrible_amazon_aws_iam_role_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_amazon_aws_iam_role_info (Resource)
...
- `name` (String) Name of a role to search for. Mutually exclusive with `path_prefix`.
- `path_prefix` (String) Prefix of role to restrict IAM role search for. Mutually exclusive with `name`. `path` and `prefix` were added as aliases in release 7.2.0. In a release after 2026-05-01 paths must begin and end with `/`. Prior to this paths will automatically have `/` added as appropriate to ensure that they start and end with `/`.
...
- `iam_roles` (String) List of IAM roles.
