...
page_title: "terrible_azure_azcollection_azure_rm_devtestlab Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_azure_azcollection_azure_rm_devtestlab (Resource)
...
- `premium_data_disks` (Boolean) Allow creation of premium data disks.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) Assert the state of the DevTest Lab. Use `present` to create or update an DevTest Lab and `absent` to delete it.
- `storage_type` (String) Type of storage used by the lab. It can be either `premium` or `standard`.
