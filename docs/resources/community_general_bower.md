...
page_title: "terrible_community_general_bower Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_general_bower (Resource)
...
- `path` (String) The base path where to install the bower packages.
...
- `offline` (Boolean) Install packages from local cache, if the packages were installed before.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `production` (Boolean) Install with `--production` flag.
- `relative_execpath` (String) Relative path to bower executable from install path.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) The state of the bower package.
