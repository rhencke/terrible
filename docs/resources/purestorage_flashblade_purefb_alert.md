...
page_title: "terrible_purestorage_flashblade_purefb_alert Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_purestorage_flashblade_purefb_alert (Resource)
...
- `address` (String) Email address (valid format required)
...
- `enabled` (Boolean) Set specified email address to be enabled or disabled
...
- `severity` (String) The minimum severity that an alert must have in order for emails to be sent to the array's alert watchers
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) Create, delete or test alert email
