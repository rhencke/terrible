...
page_title: "terrible_cisco_dnac_wireless_provision_ssid_create_provision Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_dnac_wireless_provision_ssid_create_provision (Resource)
...
- `enableFabric` (Boolean) Enable SSID for Fabric.
...
- `flexConnect` (String) Wireless Provision Ssid Create Provision's flexConnect.
- `headers` (String) Additional headers.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `managedAPLocations` (String) Managed AP Locations (Enter entire Site(s) hierarchy).
...
- `ssidDetails` (String) Wireless Provision Ssid Create Provision's ssidDetails.
