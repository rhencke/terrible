...
page_title: "terrible_cisco_mso_mso_schema_template_deploy Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_mso_mso_schema_template_deploy (Resource)
...
- `site` (String) The name of the site **to undeploy**.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) Use `deploy` to deploy schema template. Use `status` to get deployment status. Use `undeploy` to deploy schema template from a site.
