...
page_title: "terrible_cisco_dnac_http_write_credential Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_dnac_http_write_credential (Resource)
...
- `comments` (String) Comments to identify the HTT`S` Write credential.
- `credentialType` (String) Credential type to identify the application that uses the HTT`S` Write credential.
- `delegate_to_id` (String) ID of another terrible_host to delegate execution to.
- `description` (String) Description for HTT`S` Write Credential.
...
- `instanceTenantId` (String) Deprecated.
- `instanceUuid` (String) Deprecated.
- `password` (String) HTT`S` Write Password.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `port` (Number) HTT`S` Port. Valid port should be in the range of 1 to 65535.
- `secure` (Boolean) Flag for HTTPS Write.
...
- `username` (String) HTT`S` Write Username.
