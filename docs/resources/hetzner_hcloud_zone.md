---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "terrible_hetzner_hcloud_zone Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_hetzner_hcloud_zone (Resource)





<!-- schema generated by tfplugindocs -->
## Schema

### Required

- `host_id` (String) ID of the `terrible_host` to run this task against

### Optional

- `async_seconds` (Number) Run the task asynchronously, timing out after this many seconds. 0 = synchronous (default).
- `changed_when` (String) Jinja2 expression that overrides when the task is considered changed (e.g. 'false').
- `delegate_to_id` (String) ID of another terrible_host to delegate execution to.
- `delete_protection` (Boolean) Protect the Zone from deletion.
- `environment` (String) Environment variables set for the task (dict of name→value).
- `failed_when` (String) Jinja2 expression that overrides when the task is considered failed.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `labels` (String) User-defined key-value pairs.
- `mode` (String) Mode of the Zone. Required if the Zone does not exist.
- `name` (String) Name of the Zone to manage. Only required if no Zone `id` is given or the Zone does not exist. All names with well-known public suffixes (e.g. .de, .com, .co.uk) are supported. Subdomains are not supported. The name must be in lower case and must not end with a dot. Internationalized domain names must be transcribed to Punycode representation with ACE prefix, e.g. xn--mnchen-3ya.de (münchen.de).
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `primary_nameservers` (String) Primary nameservers of the Zone. Only applicable for Zones with `mode=secondary`.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) State of the Zone. `import` is not idempotent.
- `tags` (String) Run only tasks with these Ansible tags (list of strings).
- `timeout` (Number) Override the default execution timeout (seconds). Defaults to 300.
- `triggers` (String) Arbitrary map of values; any change triggers task re-execution
- `ttl` (Number) TTL of the Zone.
- `zonefile` (String) Zone file to import. Optional if `state=present` and the Zone does not exist, ignored otherwise. Required if `state=import`.

### Read-Only

- `changed` (Boolean) Whether the task reported a change
- `hcloud_zone` (String) Zone instance.
- `id` (String) Unique task resource ID
