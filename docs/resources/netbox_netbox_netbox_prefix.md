...
page_title: "terrible_netbox_netbox_netbox_prefix Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_netbox_netbox_netbox_prefix (Resource)
...
- `data` (String) Defines the prefix configuration
...
- `first_available` (Boolean) If `yes` and state `present`, if an parent is given, it will get the first available prefix of the given prefix_length inside the given parent (and vrf, if given). Unused with state `absent`.
...
- `msg` (String) Message indicating failure or info about what has been achieved
- `prefix` (String) Serialized object as created or already existent within NetBox
