...
page_title: "terrible_cisco_nxos_nxos_udld Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_nxos_nxos_udld (Resource)
...
- `aggressive` (String) Toggles aggressive mode.
...
- `reset` (Boolean) Ability to reset all ports shut down by UDLD. 'state' parameter cannot be 'absent' when this is present.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) Manage the state of the resource. When set to 'absent', aggressive and msg_time are set to their default values.
...
- `end_state` (String) k/v pairs of udld configuration after module execution
- `existing` (String) k/v pairs of existing udld configuration
