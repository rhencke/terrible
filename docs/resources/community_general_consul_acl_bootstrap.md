...
page_title: "terrible_community_general_consul_acl_bootstrap Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_general_consul_acl_bootstrap (Resource)
...
- `result` (String) The bootstrap result as returned by the Consul HTTP API. **Note:** If `bootstrap_secret` has been specified the `SecretID` and `ID` do not contain the secret but `VALUE_SPECIFIED_IN_NO_LOG_PARAMETER`. If you pass `bootstrap_secret`, make sure your playbook/role does not depend on this return value!
