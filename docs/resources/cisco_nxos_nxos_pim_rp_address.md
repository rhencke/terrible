...
page_title: "terrible_cisco_nxos_nxos_pim_rp_address Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_nxos_nxos_pim_rp_address (Resource)
...
- `rp_address` (String) Configures a Protocol Independent Multicast (PIM) static rendezvous point (RP) address. Valid values are unicast addresses.

### Optional

- `async_seconds` (Number) Run the task asynchronously, timing out after this many seconds. 0 = synchronous (default).
- `bidir` (Boolean) Group range is treated in PIM bidirectional mode.
