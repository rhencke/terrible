...
page_title: "terrible_ansible_windows_win_firewall Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_ansible_windows_win_firewall (Resource)
...
- `state` (String) Set state of firewall for given profile.
...
- `inbound_action` (String) Set to `allow` or `block` inbound network traffic in the profile. `not_configured` is valid when configuring a GPO.
- `outbound_action` (String) Set to `allow` or `block` inbound network traffic in the profile. `not_configured` is valid when configuring a GPO.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `profiles` (String) Specify one or more profiles to change.
...
- `enabled` (Boolean) Current firewall status for chosen profile (after any potential change).
