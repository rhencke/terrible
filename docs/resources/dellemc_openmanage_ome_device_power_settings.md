...
page_title: "terrible_dellemc_openmanage_ome_device_power_settings Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_dellemc_openmanage_ome_device_power_settings (Resource)
...
- `device_id` (Number) The ID of the chassis for which the settings need to be updated. If the device ID is not specified, this module updates the power settings for the *hostname*. *device_id* is mutually exclusive with *device_service_tag*.
...
- `hot_spare_configuration` (String) The settings for Hot Spare configuration.
...
- `power_configuration` (String) The settings for Power configuration.
- `redundancy_configuration` (String) The settings for Redundancy configuration.
...
- `msg` (String) Overall status of the device power settings.
- `power_details` (String) returned when power settings are updated successfully.
