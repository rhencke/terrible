...
page_title: "terrible_community_docker_docker_swarm_service_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_docker_docker_swarm_service_info (Resource)
...
- `name` (String) The name of the service to inspect.
...
- `exists` (Boolean) Returns whether the service exists.
- `id` (String) Unique task resource ID
- `service` (String) A dictionary representing the current state of the service. Matches the `docker service inspect` output. Will be `none` if service does not exist.
