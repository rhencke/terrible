...
page_title: "terrible_ieisystem_inmanage_user Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_ieisystem_inmanage_user (Resource)
...
- `priv` (String) Other user permissions, select one or more from None/KVM/VMM/SOL.
- `role_id` (String) User group. Default user group 'Administrator', 'Operator', 'User'. Use command `user_group_info` can get all group information.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) Whether the user should exist or not, taking action if the state is different from what is stated.
...
- `uid` (Number) User id, The range is 1 to 16.
