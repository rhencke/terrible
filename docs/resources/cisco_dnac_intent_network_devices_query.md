...
page_title: "terrible_cisco_dnac_intent_network_devices_query Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_dnac_intent_network_devices_query (Resource)
...
- `filter` (String) Intent Network Devices Query's filter.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `page` (String) Intent Network Devices Query's page.
...
- `views` (String) The specific views being requested. This is an optional parameter which can be passed to get one or more of the network device data. If this is not provided, then it will default to BASIC views. If multiple views are provided, the response will contain the union of the views.
