...
page_title: "terrible_community_vmware_vmware_custom_attribute_manager Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_vmware_vmware_custom_attribute_manager (Resource)
...
- `custom_attributes` (String) A list of name and value of custom attributes that needs to be manage. Value of custom attribute is not required and will be ignored if `state=absent`.
- `host_id` (String) ID of the `terrible_host` to run this task against
- `object_name` (String) Name of the vSphere object to work with.
- `object_type` (String) Type of the object the custom attribute is associated with.
...
- `state` (String) If set to `present`, the custom attribute is set to the given value. If set to `absent`, the custom attribute is cleared. The given value is ignored in this case.
