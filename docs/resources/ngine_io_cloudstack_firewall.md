---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "terrible_ngine_io_cloudstack_firewall Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_ngine_io_cloudstack_firewall (Resource)





<!-- schema generated by tfplugindocs -->
## Schema

### Required

- `host_id` (String) ID of the `terrible_host` to run this task against
- `zone` (String) Name of the zone in which the virtual machine is in.

### Optional

- `account` (String) Account the firewall rule is related to.
- `async_seconds` (Number) Run the task asynchronously, timing out after this many seconds. 0 = synchronous (default).
- `changed_when` (String) Jinja2 expression that overrides when the task is considered changed (e.g. 'false').
- `cidrs` (String) List of CIDRs (full notation) to be used for firewall rule. Since version 2.5, it is a list of CIDR.
- `delegate_to_id` (String) ID of another terrible_host to delegate execution to.
- `domain` (String) Domain the firewall rule is related to.
- `end_port` (Number) End port for this rule. Considered if *protocol=tcp* or *protocol=udp*. If not specified, equal *start_port*.
- `environment` (String) Environment variables set for the task (dict of name→value).
- `failed_when` (String) Jinja2 expression that overrides when the task is considered failed.
- `icmp_code` (Number) Error code for this icmp message. Considered if *protocol=icmp*.
- `icmp_type` (Number) Type of the icmp message being sent. Considered if *protocol=icmp*.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `ip_address` (String) Public IP address the ingress rule is assigned to. Required if *type=ingress*.
- `network` (String) Network the egress rule is related to. Required if *type=egress*.
- `poll_async` (Boolean) Poll async jobs until job has finished.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `project` (String) Name of the project the firewall rule is related to.
- `protocol` (String) Protocol of the firewall rule. `all` is only available if *type=egress*.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `start_port` (Number) Start port for this rule. Considered if *protocol=tcp* or *protocol=udp*.
- `state` (String) State of the firewall rule.
- `tags` (String) Run only tasks with these Ansible tags (list of strings).
- `timeout` (Number) Override the default execution timeout (seconds). Defaults to 300.
- `triggers` (String) Arbitrary map of values; any change triggers task re-execution
- `type` (String) Type of the firewall rule.

### Read-Only

- `changed` (Boolean) Whether the task reported a change
- `cidr` (String) CIDR string of the rule.
- `id` (String) Unique task resource ID
