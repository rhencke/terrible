...
page_title: "terrible_infoblox_nios_modules_nios_restartservices Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_infoblox_nios_modules_nios_restartservices (Resource)
...
- `groups` (String) The list of the Service Restart Groups to restart.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `members` (String) The list of the Grid Members to restart.
- `mode` (String) The restart method in case of grid restart.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `restart_option` (String) Controls whether services are restarted unconditionally or when needed
- `services` (String) The list of services the restart applicable to.
