...
page_title: "terrible_check_point_mgmt_cp_mgmt_smart_task Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_check_point_mgmt_cp_mgmt_smart_task (Resource)
...
- `action` (String) The action to be run when the trigger is fired.
...
- `enabled` (Boolean) Whether the SmartTask is enabled and will run when triggered.
- `environment` (String) Environment variables set for the task (dict of name→value).
- `fail_open` (Boolean) If the action fails to execute, whether to treat the execution failure as an error, or continue.
...
- `cp_mgmt_smart_task` (String) The checkpoint object created or updated.
