...
page_title: "terrible_dellemc_enterprise_sonic_sonic_pim_interfaces Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_dellemc_enterprise_sonic_sonic_pim_interfaces (Resource)
...
- `config` (String) Specifies interface-specific PIM configurations.
...
- `state` (String) The state of the configuration after module completion. `merged` - Merges provided interface-specific PIM configuration with on-device configuration. `replaced` - Replaces on-device PIM configuration of the specified interfaces with provided configuration. `overridden` - Overrides all on-device interface-specific PIM configurations with the provided configuration. `deleted` - Deletes on-device interface-specific PIM configuration.
...
- `after` (String) The resulting configuration on module invocation.
- `after(generated)` (String) The generated configuration on module invocation.
