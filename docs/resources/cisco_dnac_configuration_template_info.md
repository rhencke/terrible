...
page_title: "terrible_cisco_dnac_configuration_template_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_dnac_configuration_template_info (Resource)
...
- `latestVersion` (Boolean) LatestVersion query parameter. LatestVersion flag to get the latest versioned template.
...
- `projectNames` (String) ProjectNames query parameter. Filter template(s) based on project names.
...
- `unCommitted` (Boolean) UnCommitted query parameter. Filter template(s) based on template commited or not.
