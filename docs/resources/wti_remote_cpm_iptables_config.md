...
page_title: "terrible_wti_remote_cpm_iptables_config Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_wti_remote_cpm_iptables_config (Resource)
...
- `command` (String) Actual iptables command to send to the WTI device.
...
- `clear` (Number) Removes all the iptables for the protocol being defined before setting the newly defined entry.
...
- `index` (String) Index in which command should be inserted. If not defined entry will start at position one.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `protocol` (Number) The protocol that the iptables entry should be applied. 0 = ipv4, 1 = ipv6.
