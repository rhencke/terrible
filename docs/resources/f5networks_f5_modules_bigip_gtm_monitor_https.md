...
page_title: "terrible_f5networks_f5_modules_bigip_gtm_monitor_https Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_f5networks_f5_modules_bigip_gtm_monitor_https (Resource)
...
- `cipher_list` (String) Specifies the list of ciphers for this monitor. The items in the cipher list are separated with the colon `:` symbol. When creating a new monitor, if this parameter is not specified, the default list is `DEFAULT:+SHA:+3DES:+kEDH`.
- `client_cert` (String) Specifies a fully-qualified path for a client certificate the monitor sends to the target SSL server.
- `client_key` (String) Specifies a key for a client certificate the monitor sends to the target SSL server.
- `compatibility` (Boolean) When enabled, specifies the SSL options setting (in OpenSSL) is set to **all**. When creating a new monitor, if this value is not specified, the default is `true`
...
- `interval` (Number) The interval specifying how frequently the monitor instance of this template will run. If this parameter is not provided when creating a new monitor, the default value is 30. This value **must** be less than the `timeout` value.
...
- `parent` (String) The parent template of this monitor template. Once this value has been set, it cannot be changed. By default, this value is the `https` parent on the `Common` partition.
...
- `probe_timeout` (Number) Specifies the number of seconds after which the system times out the probe request to the system. When creating a new monitor, if this parameter is not provided, then the default value is `5`.
