...
page_title: "terrible_azure_azcollection_azure_rm_expressroute Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_azure_azcollection_azure_rm_expressroute (Resource)
...
- `allow_classic_operations` (Boolean) Support for classic operations.
- `async_seconds` (Number) Run the task asynchronously, timing out after this many seconds. 0 = synchronous (default).
- `authorizations` (String) The list of authorizations.
...
- `service_provider_properties` (String) The service Provider properties
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `sku` (String) The name of the SKU. Please see [https://azure.microsoft.com/en-in/pricing/details/expressroute/]() Required sku when *state=present*.
- `state` (String) Assert the state of the express route. Use `present` to create or update an express route and `absent` to delete it.
