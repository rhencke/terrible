...
page_title: "terrible_cisco_iosxr_iosxr_route_maps Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_iosxr_iosxr_route_maps (Resource)
...
- `config` (String) A list of configurations for route policy.
...
- `running_config` (String) This option is used only with state *parsed*. The value of this option should be and aggregate of the output received from the IOS XR device by executing the command **show running-config route-policy <policy_name>** for per route-policy. The state *parsed* reads the configuration from `running_config` option and transforms it into Ansible structured data as per the resource module's argspec and the value is then returned in the *parsed* key within the result.
