...
page_title: "terrible_cisco_dnac_inventory_workflow_manager Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_dnac_inventory_workflow_manager (Resource)
...
- `config` (String) List of device configurations for inventory operations
...
- `config_verify` (Boolean) Enable verification of Cisco Catalyst Center configuration after applying playbook changes When enabled, the module will validate that changes were applied correctly
...
- `state` (String) Desired state of the device inventory after module execution `merged` - Add new devices or update existing device configurations `deleted` - Remove devices from inventory (use clean_config for configuration cleanup)
