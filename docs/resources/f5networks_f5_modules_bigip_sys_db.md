...
page_title: "terrible_f5networks_f5_modules_bigip_sys_db Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_f5networks_f5_modules_bigip_sys_db (Resource)
...
- `key` (String) The database variable to manipulate.
...
- `state` (String) The state of the variable on the system. When `present`, guarantees an existing variable is set to `value`. When `reset`, sets the variable back to the default value. At least one of value and state `reset` are required.
...
- `default_value` (String) The default value of the key.
- `id` (String) Unique task resource ID
- `name` (String) The key in the system database.
