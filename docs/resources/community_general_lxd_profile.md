...
page_title: "terrible_community_general_lxd_profile Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_general_lxd_profile (Resource)
...
- `merge_profile` (Boolean) Merge the configuration of the present profile with the new desired configuration, instead of replacing it.
- `new_name` (String) A new name of a profile. If this parameter is specified a profile is renamed to this name. See https://documentation.ubuntu.com/lxd/en/latest/api/#/profiles/profile_post.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `project` (String) Project of a profile. See https://documentation.ubuntu.com/lxd/en/latest/projects/.
...
- `state` (String) Define the state of a profile.
...
- `actions` (String) List of actions performed for the profile.
...
- `old_state` (String) The old state of the profile.
