...
page_title: "terrible_purestorage_flashblade_purefb_snmp_mgr Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_purestorage_flashblade_purefb_snmp_mgr (Resource)
...
- `name` (String) Name of SNMP Manager
...
- `auth_protocol` (String) SNMP v3 only. Hash algorithm to use
...
- `notification` (String) Action to perform on event.
...
- `privacy_protocol` (String) SNMP v3 only. Encryption protocol to use
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) Create, delete or test SNMP manager
