...
page_title: "terrible_dellemc_openmanage_ome_powerstate Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_dellemc_openmanage_ome_powerstate (Resource)
...
- `power_state` (String) Desired end power state.
...
- `device_id` (Number) Targeted device id. *device_id* is mutually exclusive with *device_service_tag*.
...
- `job_status` (String) Power state operation job and progress details from the OME.
- `msg` (String) Overall power state operation job status.
