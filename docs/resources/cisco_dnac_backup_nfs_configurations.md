...
page_title: "terrible_cisco_dnac_backup_nfs_configurations Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_dnac_backup_nfs_configurations (Resource)
...
- `nfsPort` (Number) NFS Port.
- `nfsVersion` (String) NFS version.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `portMapperPort` (Number) NFS port mapper port.
- `server` (String) NFS server host.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `sourcePath` (String) NFS server path.
