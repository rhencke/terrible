...
page_title: "terrible_openstack_cloud_federation_idp Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_openstack_cloud_federation_idp (Resource)
...
- `is_enabled` (Boolean) Whether the identity provider is enabled or not. Will default to `false` when creating a new identity provider.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `remote_ids` (String) List of the unique identity provider's remote IDs. Will default to an empty list when creating a new identity provider.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) Whether the identity provider should be `present` or `absent`.
...
- `identity_provider` (String) Dictionary describing the identity providers
