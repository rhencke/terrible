...
page_title: "terrible_ansible_netcommon_net_get Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_ansible_netcommon_net_get (Resource)
...
- `src` (String) Specifies the source file. The path to the source file can either be the full path on the network device or a relative path as per path supported by destination network device.
...
- `dest` (String) Specifies the destination file. The path to the destination file can either be the full path on the Ansible control host or a relative path from the playbook or role root directory.
