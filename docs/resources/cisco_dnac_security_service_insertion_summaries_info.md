...
page_title: "terrible_cisco_dnac_security_service_insertion_summaries_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_dnac_security_service_insertion_summaries_info (Resource)
...
- `limit` (Number) Limit query parameter. Maximum number of records to return. Default value is 500, minimum value is 1 and maximum value is 500.
- `offset` (Number) Offset query parameter. Starting record for pagination. The first record is numbered 1.
- `order` (String) Order query parameter. The sorting order for the response can be specified as either ascending (asc) or descending (desc). If no value is specified, the default order is ascending (asc).
