...
page_title: "terrible_dellemc_enterprise_sonic_sonic_ptp_port_ds Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_dellemc_enterprise_sonic_sonic_ptp_port_ds (Resource)
...
- `config` (String) Specifies port-specific PTP configurations.
...
- `state` (String) The state of the configuration after module completion. `merged` - Merges provided interface-specific PTP configuration with on-device configuration. `replaced` - Replaces on-device PTP configuration of the specified interfaces with provided configuration. `overridden` - Overrides all on-device interface-specific PTP configurations with the provided configuration. `deleted` - Deletes on-device interface-specific PTP configuration.
