...
page_title: "terrible_amazon_aws_ec2_ami_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_amazon_aws_ec2_ami_info (Resource)
...
- `describe_image_attributes` (Boolean) Describe attributes (like launchPermission) of the images found.
- `environment` (String) Environment variables set for the task (dict of name→value).
- `executable_users` (String) Filter images by users with explicit launch permissions. Valid options are an AWS account ID, self, or all (public AMIs).
- `failed_when` (String) Jinja2 expression that overrides when the task is considered failed.
- `filters` (String) A dict of filters to apply. Each dict item consists of a filter key and a filter value. See https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeImages.html for possible filters. Filter names and values are case sensitive.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `image_ids` (String) One or more image IDs.
- `owners` (String) Filter the images by the owner. Valid options are an AWS account ID, self, or an AWS owner alias ( amazon | aws-marketplace | microsoft ).
...
- `images` (String) A list of images.
