...
page_title: "terrible_community_general_urpmi Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_general_urpmi (Resource)
...
- `name` (String) A list of package names to install, upgrade or remove.
...
- `force` (Boolean) Assume "yes" is the answer to any question urpmi has to ask. Corresponds to the `--force` option for `urpmi`.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `no_recommends` (Boolean) Corresponds to the `--no-recommends` option for `urpmi`.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `root` (String) Specifies an alternative install root, relative to which all packages are installed. Corresponds to the `--root` option for `urpmi`.
...
- `update_cache` (Boolean) Update the package database first `urpmi.update -a`.
