...
page_title: "terrible_check_point_mgmt_cp_mgmt_install_software_package Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_check_point_mgmt_cp_mgmt_install_software_package (Resource)
...
- `cluster_installation_settings` (String) Installation settings for cluster.
- `concurrency_limit` (Number) The number of targets, on which the same package is installed at the same time.
...
- `method` (String) NOTE, Supported from Check Point version R81 How we want to use the package.
- `name` (String) The name of the software package.
- `package_location` (String) NOTE, Supported from Check Point version R81 The package repository.
...
- `cp_mgmt_install_software_package` (String) The checkpoint install-software-package output.
