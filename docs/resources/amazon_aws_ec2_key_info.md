...
page_title: "terrible_amazon_aws_ec2_key_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_amazon_aws_ec2_key_info (Resource)
...
- `filters` (String) A dict of filters to apply. Each dict item consists of a filter key and a filter value. See https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeKeyPairs.html for possible filters. Filter names and values are case sensitive.
- `ids` (String) The IDs of the key pairs.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `include_public_key` (Boolean) Whether or not to include the public key material in the response.
- `names` (String) The key pair names.
...
- `keypairs` (String) A list of ec2 key pairs.
