...
page_title: "terrible_community_general_manageiq_tags Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_general_manageiq_tags (Resource)
...
- `resource_type` (String) The relevant resource type in manageiq.
...
- `resource_id` (Number) The ID of the resource at which tags are controlled. Must be specified if `resource_name` is not set. Both options are mutually exclusive.
- `resource_name` (String) The name of the resource at which tags are be controlled. Must be specified if `resource_id` is not set. Both options are mutually exclusive.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) `absent` - tags should not exist, `present` - tags should exist.
