...
page_title: "terrible_cisco_mso_mso_schema_template Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_mso_mso_schema_template (Resource)
...
- `state` (String) Use `present` or `absent` for adding or removing. Use `query` for listing an object or multiple objects. Using `present` on empty schemas `cisco.mso.mso_schema` is supported on versions of MSO that are 4.2 or greater.
...
- `tenant` (String) The tenant used for this template.
