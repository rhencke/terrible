...
page_title: "terrible_junipernetworks_junos_junos_rpc Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_junipernetworks_junos_junos_rpc (Resource)
...
- `rpc` (String) The `rpc` argument specifies the RPC call to send to the remote devices to be executed.  The RPC Reply message is parsed and the contents are returned to the playbook.

### Optional

- `args` (String) The `args` argument provides a set of arguments for the RPC call and are encoded in the request message.  This argument accepts a set of key=value arguments.
- `async_seconds` (Number) Run the task asynchronously, timing out after this many seconds. 0 = synchronous (default).
- `attrs` (String) The `attrs` arguments defines a list of attributes and their values to set for the RPC call. This accepts a dictionary of key-values.
...
- `output` (String) The `output` argument specifies the desired output of the return data.  This argument accepts one of `xml`, `text`, or `json`.  For `json`, the JUNOS device must be running a version of software that supports native JSON output.
...
- `output_lines` (String) The text output split into lines for readability.
- `xml` (String) The xml return string from the rpc request.
