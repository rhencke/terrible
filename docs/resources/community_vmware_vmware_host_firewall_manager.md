...
page_title: "terrible_community_vmware_vmware_host_firewall_manager Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_vmware_vmware_host_firewall_manager (Resource)
...
- `rules` (String) A list of Rule set which needs to be managed. Each member of list is rule set name and state to be set the rule. Both rule name and rule state are required parameters. Additional IPs and networks can also be specified Please see examples for more information.
...
- `rule_set_state` (String) dict with hostname as key and dict with firewall rule set facts as value
