...
page_title: "terrible_dellemc_openmanage_idrac_lifecycle_controller_logs Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_dellemc_openmanage_idrac_lifecycle_controller_logs (Resource)
...
- `share_name` (String) Network share or local path. CIFS, NFS network share types are supported.
...
- `job_wait` (Boolean) Whether to wait for the running job completion or not.
...
- `lc_logs_status` (String) Status of the export operation along with job details and file path.
- `msg` (String) Status of the export lifecycle controller logs job.
