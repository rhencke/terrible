...
page_title: "terrible_ieisystem_inmanage_ad_group Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_ieisystem_inmanage_ad_group (Resource)
...
- `name` (String) Enter the Role Group Name. This name identifies the role group in Active Directory.
...
- `domain` (String) Enter the Role Group Domain. This is the domain where the role group is located.
...
- `kvm` (String) This field provides access to KVM for AD authenticated role group user.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `pri` (String) Enter the Role Group Privilege. This is the level of privilege to be assigned for this role group.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) Whether the active directory group should exist or not, taking action if the state is different from what is stated.
...
- `vm` (String) This field provides access to VMedia for AD authenticated role group user.
