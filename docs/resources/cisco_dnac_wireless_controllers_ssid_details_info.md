...
page_title: "terrible_cisco_dnac_wireless_controllers_ssid_details_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_dnac_wireless_controllers_ssid_details_info (Resource)
...
- `adminStatus` (Boolean) AdminStatus query parameter. Utilize this query parameter to obtain the administrative status. A 'true' value signifies that the admin status of the SSID is enabled, while a 'false' value indicates that the admin status of the SSID is disabled.
...
- `networkDeviceId` (String) NetworkDeviceId path parameter. Obtain the network device ID value by using the API call GET /dna/intent/api/v1/network-device/ip-address/${ipAddress}.
...
- `ssidName` (String) SsidName query parameter. Employ this query parameter to obtain the details of the SSID corresponding to the provided SSID name.
