...
page_title: "terrible_community_rabbitmq_rabbitmq_user_limits Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_rabbitmq_rabbitmq_user_limits (Resource)
...
- `user` (String) Name of user to manage limits for.
...
- `max_channels` (Number) Max number of channels. Negative value means "no limit". Ignored when the *state* is `absent`.
...
- `state` (String) Specify whether the limits are to be set or cleared. If set to `absent`, the limits of both *max_connections* and *max_channels* will be cleared.
