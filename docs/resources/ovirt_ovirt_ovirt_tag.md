...
page_title: "terrible_ovirt_ovirt_ovirt_tag Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_ovirt_ovirt_ovirt_tag (Resource)
...
- `name` (String) Name of the tag to manage.
...
- `hosts` (String) List of the hosts names, which should have assigned this tag.
...
- `state` (String) Should the tag be present/absent/attached/detached. `Note`: *attached* and *detached* states are supported since version 2.4.
...
- `vms` (String) List of the VMs names, which should have assigned this tag.

### Read-Only

- `changed` (Boolean) Whether the task reported a change
- `id` (String) Unique task resource ID
- `tag` (String) Dictionary of all the tag attributes. Tag attributes can be found on your oVirt/RHV instance at following url: http://ovirt.github.io/ovirt-engine-api-model/master/#types/tag.
