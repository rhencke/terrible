...
page_title: "terrible_f5networks_f5_modules_bigip_service_policy Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_f5networks_f5_modules_bigip_service_policy (Resource)
...
- `port_misuse_policy` (String) The port misuse policy to attach to the service policy. Requires `afm` (Advanced Firewall Manager) be provisioned to use. If `afm` is not provisioned, this parameter is ignored.
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) Whether the resource should exist or not.
