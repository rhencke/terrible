...
page_title: "terrible_cisco_meraki_networks_wireless_rf_profiles_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_meraki_networks_wireless_rf_profiles_info (Resource)
...
- `includeTemplateProfiles` (Boolean) Information module for Networks Wireless Rfprofiles Info. IncludeTemplateProfiles query parameter. If the network is bound to a template, this parameter controls whether or not the non-basic RF profiles defined on the template should be included in the response alongside the non-basic profiles defined on the bound network. Defaults to false.
- `networkId` (String) Information module for Networks Wireless Rfprofiles Info. NetworkId path parameter. Network ID.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `rfProfileId` (String) Information module for Networks Wireless Rfprofiles Info. RfProfileId path parameter. Rf profile ID.
