...
page_title: "terrible_community_routeros_facts Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_routeros_facts (Resource)
...
- `gather_subset` (String) When supplied, this argument will restrict the facts collected to a given subset. Possible values for this argument include `all`, `hardware`, `config`, `interfaces`, and `routing`. Can specify a list of values to include a larger subset. Values can also be used with an initial `!` to specify that a specific subset should not be collected.
