...
page_title: "terrible_community_general_atomic_image Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_general_atomic_image (Resource)
...
- `backend` (String) Define the backend where the image is pulled.
...
- `started` (Boolean) Start or stop the container.
- `state` (String) The state of the container image. The state `latest` will ensure container image is upgraded to the latest version and forcefully restart container, if running.
