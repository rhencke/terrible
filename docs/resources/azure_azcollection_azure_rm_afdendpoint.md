...
page_title: "terrible_azure_azcollection_azure_rm_afdendpoint Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_azure_azcollection_azure_rm_afdendpoint (Resource)
...
- `name` (String) Name of the AFD Endpoint.
- `profile_name` (String) Name of the AFD Profile where the Endpoint will be attached to.
- `resource_group` (String) Name of a resource group where the Azure Front Door Endpoint exists or will be created.

### Optional

- `async_seconds` (Number) Run the task asynchronously, timing out after this many seconds. 0 = synchronous (default).
- `auto_generated_domain_name_label_scope` (String) Indicates the endpoint name reuse scope. Cannot be used to update an existing Endpoint at this time.
...
- `enabled_state` (String) Whether to enable use of this rule.
...
- `location` (String) Valid Azure location. Defaults to location of the resource group. Cannot be used to update an existing Endpoint at this time.
...
- `state` (String) Assert the state of the AFD Endpoint. Use `present` to create or update an AFD Endpoint and `absent` to delete it.
...
- `host_name` (String) Host name of the AFD Endpoint.
