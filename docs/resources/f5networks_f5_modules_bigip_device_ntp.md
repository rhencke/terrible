...
page_title: "terrible_f5networks_f5_modules_bigip_device_ntp Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_f5networks_f5_modules_bigip_device_ntp (Resource)
...
- `ntp_servers` (String) A list of NTP servers to set on the device. At least one of `ntp_servers` or `timezone` is required.
...
- `state` (String) The state of the NTP servers on the system. When `present`, guarantees the NTP servers are set on the system. When `absent`, removes the specified NTP servers from the device configuration.
