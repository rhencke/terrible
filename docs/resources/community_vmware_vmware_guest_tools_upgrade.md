...
page_title: "terrible_community_vmware_vmware_guest_tools_upgrade Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_vmware_vmware_guest_tools_upgrade (Resource)
...
- `datacenter` (String) Destination datacenter where the virtual machine exists.
...
- `force_upgrade` (Boolean) This flag overrides the guest operating system detection and forcibly upgrade VMware tools or open-vm-tools. This is useful when VMware tools is too old and unable to detect the 'guestFamily' value. Using this flag may sometime give unexpected results since module will override the default behaviour of 'guestFamily' detection.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `installer_options` (String) Command line options passed to the installer to modify the installation procedure for tools.
