...
page_title: "terrible_wti_remote_cpm_firmware_update Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_wti_remote_cpm_firmware_update (Resource)
...
- `cpm_path` (String) This is the directory path to store the WTI device configuration file.
...
- `family` (Number) Force the download to both either Console (1) or Power (0)
...
- `removefileonexit` (Number) After an upgrade, remove the upgrade OS image
...
- `use_force` (Boolean) If set to True, the upgrade will happen even if the device doesnt need it.
...
- `data` (String) The output XML configuration of the WTI device being updated
