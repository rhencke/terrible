...
page_title: "terrible_community_aws_dynamodb_ttl Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_aws_dynamodb_ttl (Resource)
...
- `attribute_name` (String) The name of the Time To Live attribute used to store the expiration time for items in the table. This appears to be required by the API even when disabling TTL.
- `host_id` (String) ID of the `terrible_host` to run this task against
- `table_name` (String) Name of the DynamoDB table to work on.
...
- `state` (String) State to set DynamoDB table to.
...
- `current_status` (String) current or new TTL specification.
