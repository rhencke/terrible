...
page_title: "terrible_community_windows_win_scoop Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_windows_win_scoop (Resource)
...
- `name` (String) Name of the package(s) to be installed.

### Optional

- `architecture` (String) Force Scoop to install the package of a specific process architecture.
...
- `global` (Boolean) Install the app globally
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `independent` (Boolean) Don't install dependencies automatically
- `no_cache` (Boolean) Don't use the download cache
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `purge` (Boolean) Remove all persistent data
- `skip_checksum` (Boolean) Skip hash validation
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) State of the package on the system. When `absent`, will ensure the package is not installed. When `present`, will ensure the package is installed.
