...
page_title: "terrible_community_general_memset_dns_reload Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_general_memset_dns_reload (Resource)
...
- `poll` (Boolean) If `true`, it polls the reload job's status and return when the job has completed (unless the 30 second timeout is reached first). If the timeout is reached then the task does not return as failed, but stderr indicates that the polling failed.
...
- `memset_api` (String) Raw response from the Memset API.
