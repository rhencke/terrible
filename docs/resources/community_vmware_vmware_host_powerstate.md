...
page_title: "terrible_community_vmware_vmware_host_powerstate Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_vmware_vmware_host_powerstate (Resource)
...
- `force` (Boolean) This parameter specify if the host should be proceeding with user defined powerstate regardless of whether it is in maintenance mode. If `state=reboot-host` and `force=true`, then host system is rebooted regardless of whether it is in maintenance mode. If `state=shutdown-host` and `force=true`, then host system is shutdown regardless of whether it is in maintenance mode. If `state=power-down-to-standby` and `force=true`, then all powered off VMs will evacuated. Not applicable if `state=power-up-from-standby`.
...
- `state` (String) Set the state of the host system.
...
- `result` (String) metadata about host system's state
