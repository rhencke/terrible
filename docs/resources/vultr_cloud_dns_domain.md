...
page_title: "terrible_vultr_cloud_dns_domain Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_vultr_cloud_dns_domain (Resource)
...
- `dns_sec` (String) Ensure DNSSEC is enabled or disabled.
...
- `ip` (String) The default server IP. Use `vultr.cloud.dns_record` to change it once the domain is created. Required if `state=present`.
...
- `state` (String) State of the DNS domain.
...
- `vultr_dns_domain` (String) Response from Vultr API.
