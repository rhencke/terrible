...
page_title: "terrible_check_point_mgmt_cp_mgmt_get_interfaces Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_check_point_mgmt_cp_mgmt_get_interfaces (Resource)
...
- `group_interfaces_by_subnet` (Boolean) Specify whether to group the cluster interfaces by a subnet. Otherwise, group the cluster interfaces by their names.
...
- `use_defined_by_routes` (Boolean) Specify whether to configure the topology "Defined by Routes" where applicable. Otherwise, configure the topology to "This Network" as default for internal interfaces.
- `with_topology` (Boolean) Specify whether to fetch the interfaces with their topology. Otherwise, the Management Server fetches the interfaces without their topology.

### Read-Only

- `changed` (Boolean) Whether the task reported a change
- `cp_mgmt_get_interfaces` (String) The checkpoint get-interfaces output.
