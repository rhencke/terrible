...
page_title: "terrible_ibm_storage_virtualize_ibm_sv_manage_fcportsetmember Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_ibm_storage_virtualize_ibm_sv_manage_fcportsetmember (Resource)
...
- `fcportid` (String) Specifies the Fibre Channel I/O port ID of the port. The value can be a decimal number 1 to the maximum number of FC I/O ports.
- `host_id` (String) ID of the `terrible_host` to run this task against
- `name` (String) Specifies the name of the FC portset.
- `state` (String) Add (`present`) or Remove (`absent`) the FC port ID to or from the FC portset
