...
page_title: "terrible_azure_azcollection_azure_rm_localnetworkgateway Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_azure_azcollection_azure_rm_localnetworkgateway (Resource)
...
- `bgp_settings` (String) Local network gateway's BGP speaker settings.
...
- `gateway_ip_address` (String) IP address of local network gateway.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `local_network_address_space` (String) Local network site address space.
- `location` (String) The location of the local network gateway.
...
- `state` (String) Use `present` to create or update a local network gateway. Use `absent` to delete the local network gateway.
