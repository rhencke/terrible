...
page_title: "terrible_ansible_windows_win_feature_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_ansible_windows_win_feature_info (Resource)
...
- `name` (String) If specified, this is used to match the `name` of the Windows feature to get the info for. Can be a wildcard to match multiple features but the wildcard will only be matched on the `name` of the feature. If omitted then all features will returned.
...
- `exists` (Boolean) Whether any features were found based on the criteria specified.
- `features` (String) A list of feature(s) that were found based on the criteria. Will be an empty list if no features were found.
