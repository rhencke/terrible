...
page_title: "terrible_azure_azcollection_azure_rm_mysqlfirewallrule Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_azure_azcollection_azure_rm_mysqlfirewallrule (Resource)
...
- `name` (String) The name of the MySQL firewall rule.
...
- `end_ip_address` (String) The end IP address of the MySQL firewall rule. Must be IPv4 format. Required when creating.
...
- `start_ip_address` (String) The start IP address of the MySQL firewall rule. Must be IPv4 format. Required when creating.
- `state` (String) Assert the state of the MySQL firewall rule. Use `present` to create or update a rule and `absent` to ensure it is not present.
