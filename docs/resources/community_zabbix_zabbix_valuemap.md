...
page_title: "terrible_community_zabbix_zabbix_valuemap Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_zabbix_zabbix_valuemap (Resource)
...
- `mappings` (String) List of value mappings for the value map. Required when *state=present*.
...
- `state` (String) State of the value map. On `present`, it will create a value map if it does not exist or update the value map if the associated data is different. On `absent`, it will remove the value map if it exists.
