...
page_title: "terrible_ibm_storage_virtualize_ibm_sv_manage_provisioning_policy Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_ibm_storage_virtualize_ibm_sv_manage_provisioning_policy (Resource)
...
- `name` (String) Specifies the name of the provisioning policy. Specifies the new name during rename.
- `state` (String) Creates, updates (`present`), or deletes (`absent`) a provisioning policy.

### Optional

- `async_seconds` (Number) Run the task asynchronously, timing out after this many seconds. 0 = synchronous (default).
- `capacitysaving` (String) Specifies the policy capacity savings. Applies, when *state=present*, to create a provisioning policy.
- `changed_when` (String) Jinja2 expression that overrides when the task is considered changed (e.g. 'false').
- `deduplicated` (Boolean) Specifies when volumes should be deduplicated. Applicable when *capacitysaving=thin* or *capacitysaving=compressed*.
