...
page_title: "terrible_cisco_dnac_wireless_access_points_provision Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_dnac_wireless_access_points_provision (Resource)
...
- `apZoneName` (String) AP Zone Name. A custom AP Zone should be passed if no rfProfileName is provided.
...
- `networkDevices` (String) Wireless Access Points Provision's networkDevices.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `rfProfileName` (String) RF Profile Name. RF Profile is not allowed for custom AP Zones.
