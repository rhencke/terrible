...
page_title: "terrible_cisco_meraki_networks_switch_storm_control Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_meraki_networks_switch_storm_control (Resource)
...
- `broadcastThreshold` (Number) Percentage (1 to 99) of total available port bandwidth for broadcast traffic type. Default value 100 percent rate is to clear the configuration.
...
- `multicastThreshold` (Number) Percentage (1 to 99) of total available port bandwidth for multicast traffic type. Default value 100 percent rate is to clear the configuration.
...
- `treatTheseTrafficTypesAsOneThreshold` (String) Grouped traffic types.
- `triggers` (String) Arbitrary map of values; any change triggers task re-execution
- `unknownUnicastThreshold` (Number) Percentage (1 to 99) of total available port bandwidth for unknown unicast (dlf-destination lookup failure) traffic type. Default value 100 percent rate is to clear the configuration.
