...
page_title: "terrible_purestorage_flasharray_purefa_messages Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_purestorage_flasharray_purefa_messages (Resource)
...
- `flagged` (Boolean) Show alerts that have been acknowledged or not
- `history` (String) Historical time period to show alerts for, from present time Allowed time period are hour(h), day(d), week(w) and year(y)
...
- `severity` (String) severity of the alerts to show
- `skip_tags` (String) Skip tasks with these Ansible tags (list of strings).
- `state` (String) State of alerts to show
