...
page_title: "terrible_community_hrobot_server_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_hrobot_server_info (Resource)
...
- `full_info` (Boolean) Whether to provide full information for every server. Setting this to `true` requires one REST call per server, which is slow and reduces your rate limit. Use with care. When `server_number` is specified, this option is always treated as having value `true`.
...
- `server_number` (Number) Limit result list to server with this number.
...
- `servers` (String) List of servers matching the provided options.
