...
page_title: "terrible_dellemc_enterprise_sonic_sonic_bgp_neighbors_af Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_dellemc_enterprise_sonic_sonic_bgp_neighbors_af (Resource)
...
- `config` (String) Specifies the BGP neighbors address-family related configuration.
...
- `state` (String) Specifies the operation to be performed on the BGP process that is configured on the device. In case of merged, the input configuration is merged with the existing BGP configuration on the device. In case of deleted, the existing BGP configuration is removed from the device.
