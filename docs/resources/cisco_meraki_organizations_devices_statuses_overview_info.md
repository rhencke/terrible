...
page_title: "terrible_cisco_meraki_organizations_devices_statuses_overview_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_meraki_organizations_devices_statuses_overview_info (Resource)
...
- `networkIds` (String) Information module for Organizations Devices Statuses Overview Info. NetworkIds query parameter. An optional parameter to filter device statuses by network.
- `organizationId` (String) Information module for Organizations Devices Statuses Overview Info. OrganizationId path parameter. Organization ID.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `productTypes` (String) Information module for Organizations Devices Statuses Overview Info. ProductTypes query parameter. An optional parameter to filter device statuses by product type. Valid types are wireless, appliance, switch, systemsManager, camera, cellularGateway, sensor, wirelessController, campusGateway, and secureConnect.
