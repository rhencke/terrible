...
page_title: "terrible_community_aws_directconnect_gateway Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_aws_directconnect_gateway (Resource)
...
- `amazon_asn` (String) The Amazon side ASN. Required when *state=present*.
...
- `direct_connect_gateway_id` (String) The ID of an existing Direct Connect Gateway. Required when *state=absent*.
...
- `state` (String) Set *state=present* to ensure a resource is created. Set *state=absent* to remove a resource.
...
- `virtual_gateway_id` (String) The VPN gateway ID of an existing virtual gateway.
- `wait_timeout` (Number) How long to wait for the association to be deleted.

### Read-Only

- `changed` (Boolean) Whether the task reported a change
- `id` (String) Unique task resource ID
- `result` (String) The attributes of the Direct Connect Gateway
