...
page_title: "terrible_community_general_ipify_facts Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_community_general_ipify_facts (Resource)
...
- `api_url` (String) URL of the ipify.org API service. `?format=json` is appended by default.
...
- `validate_certs` (Boolean) When set to `false`, SSL certificates are not validated.

### Read-Only

- `changed` (Boolean) Whether the task reported a change
- `id` (String) Unique task resource ID
- `ipify_public_ip` (String) Public IP of the internet gateway.
