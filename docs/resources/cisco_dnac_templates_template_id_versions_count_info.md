...
page_title: "terrible_cisco_dnac_templates_template_id_versions_count_info Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_dnac_templates_template_id_versions_count_info (Resource)
...
- `latestVersion` (Boolean) LatestVersion query parameter. Filter response to only include the latest version of a template.
...
- `templateId` (String) TemplateId path parameter. The id of the template to get versions of, retrieveable from `GET /dna/intent/api/v1/templates`.
- `timeout` (Number) Override the default execution timeout (seconds). Defaults to 300.
- `triggers` (String) Arbitrary map of values; any change triggers task re-execution
- `versionNumber` (Number) VersionNumber query parameter. Filter response to only get the template version that matches this version number.
