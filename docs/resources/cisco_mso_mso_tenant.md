...
page_title: "terrible_cisco_mso_mso_tenant Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_cisco_mso_mso_tenant (Resource)
...
- `orchestrator_only` (String) Orchestrator Only `no` is used to delete the tenant from the MSO and Sites/APIC. `yes` is used to remove the tenant only from the MSO.
- `poll_interval` (Number) Polling interval in seconds when async_seconds > 0. Defaults to 15.
- `remote_users` (String) A list of associated remote users for this tenant.
- `sites` (String) A list of associated sites for this tenant. Using this property will replace any existing associated sites.
...
- `users` (String) A list of associated users for this tenant. Using this property will replace any existing associated users. Admin user is always added to the associated user list irrespective of this parameter being used.
