...
page_title: "terrible_dellemc_powerflex_snapshot Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_dellemc_powerflex_snapshot (Resource)
...
- `allow_multiple_mappings` (Boolean) Specifies whether to allow multiple mappings or not.
...
- `read_only` (Boolean) Specifies whether mapping of the created snapshot volume will have read-write access or limited to read-only access. If `true`, snapshot is created with read-only access. If `false`, snapshot is created with read-write access.
