...
page_title: "terrible_ibm_storage_virtualize_ibm_svc_manage_consistgrp_flashcopy Resource - terrible"
subcategory: ""
description: |-
  
---

# terrible_ibm_storage_virtualize_ibm_svc_manage_consistgrp_flashcopy (Resource)
...
- `name` (String) Specifies the name of the FlashCopy consistency group.
- `state` (String) Creates (`present`) or removes (`absent`) a FlashCopy consistency group.
...
- `force` (Boolean) If specified True, removes all the associated FlashCopy mappings while deleting the FlashCopy consistency group. Valid when *state=absent*, to delete a FlashCopy consistency group.
- `ignore_errors` (Boolean) When true, a failed task does not raise a Terraform error.
- `log_path` (String) Path of debug log file.
- `noownershipgroup` (Boolean) If specified True, the consistency group is removed from all associated ownership groups. Parameters *noownershipgroup* and *ownershipgroup* are mutually exclusive. Valid when *state=present*, to modify a FlashCopy consistency group.
